...
function loadhash (id) {
	...
	if (item.weaponCategory) {
		...
		$(".stats-wrapper .damage").html("AC "+item.ac+(type === "LA" ? " + Dex" : type === "MA" ? " + Dex (max 2)" : ""));
	} else if (type === "S") {
		...
		if (capacity) {
			...
			if (capacity.indexOf("ton") === -1 && capacity.indexOf("passenger") === -1) $(".stats-wrapper .damage").append(capacity == 1 ? " lb." : " lbs.");
		}
	}
	...
	$(".stats-wrapper .text").html(utils_makeRoller(renderStack.join("")).split(item.name.toLowerCase()).join("<i>"+item.name.toLowerCase()+"</i>"));
