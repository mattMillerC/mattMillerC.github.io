...
(() => {
    ...
    function onJsonLoad(data) {
        ...
        const timeFilter = new Filter({
            header: "Cast Time",
            items: [TM_ACTION, TM_B_ACTION, TM_REACTION, TM_ROUND, TM_MINS, TM_HRS],
            displayFn: getTimeDisplay
        });
        const rangeFilter = new Filter({
            header: "Range",
            items: [F_RNG_SELF, F_RNG_TOUCH, F_RNG_POINT, F_RNG_AREA, F_RNG_SPECIAL]
        });
        ...
        for (let i = 0; i < spellList.length; i++) {
            ...
            spell._fSubclasses = spell.classes.fromSubclass
                ? spell.classes.fromSubclass.map(c => getClassFilterStr(c.subclass))
                : [];
            ...
            tempString += `
			<tr class="table-row history-link" data-link="${encodeForHash([spell.name, spell.source])}" data-title="${
                spell.name
            }" ${FLTR_ID}="${i}" id='${i}'>
				...
				<td class='${CLS_COL_5}' title="${Parser.spSchoolAbvToFull(spell.school)}">${Parser.spSchoolAbvToFull(
                spell.school
            )}</td>
        ...
        function handleFilterChange() {
            list.filter(function(item) {
                ...
                if (filterId) {
                    ...
                    return (
                        sourceFilter.toDisplay(f, s.source) &&
                        levelFilter.toDisplay(f, s.level) &&
                        metaFilter.toDisplay(f, s._fMeta) &&
                        schoolFilter.toDisplay(f, s.school) &&
                        timeFilter.toDisplay(f, s._fTimeType) &&
                        rangeFilter.toDisplay(f, s._fRangeType) &&
                        classFilter.toDisplay(f, s._fClasses) &&
                        subclassFilter.toDisplay(f, s._fSubclasses)
                    );
                } else {
                    return true;
                }
    ...
    window.loadhash = function(id) {
        ...
        renderStack.push(
            `<div class="margin-bottom_med"><span class="stats-source source${
                spell.source
            }" title="${Parser.sourceJsonToFull(spell.source)}">${Parser.sourceJsonToAbv(spell.source)}</div>`
        );

        renderStack.push(
            `<div class="margin-bottom_med"><span>${Parser.spLevelSchoolMetaToFull(
                spell.level,
                spell.school,
                spell.meta
            )}</span></div>`
        ...
        renderStack.push(
            `<div class="margin-bottom_med"><span class="stat-name">Casting Time: </span>${Parser.spTimeListToFull(
                spell.time
            )}</div>`
        );

        renderStack.push(
            `<div class="margin-bottom_med"><span class="stat-name">Range: </span>${Parser.spRangeToFull(
                spell.range
            )}</div>`
        );

        renderStack.push(
            `<div class="margin-bottom_med"><span class="stat-name">Components: </span>${Parser.spComponentsToFull(
                spell.components
            )}</div>`
        );

        renderStack.push(
            `<div class="margin-bottom_med"><span class="stat-name">Duration: </span>${Parser.spDurationToFull(
                spell.duration
            )}</div>`
        );
        ...
        renderStack.push(
            `<div class="margin-bottom_med"><span class="stat-name">Classes: </span>${Parser.spMainClassesToFull(
                spell.classes
            )}</div>`
        );
        ...
        if (spell.classes.fromSubclass) {
            ...
            renderStack.push(
                `<div class="margin-bottom_med"><span class="stat-name">Subclasses: </span>${currentAndLegacy[0]}</div>`
            );
            if (currentAndLegacy[1]) {
                renderStack.push(
                    `<div class="mdc-theme--text-disabled-on-background margin-bottom_med"><span class="stat-name">Subclasses (legacy): </span>${
                        currentAndLegacy[1]
                    }</div>`
                );
            }
