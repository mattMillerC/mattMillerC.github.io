...
(() => {
    ...
    function onJsonLoad(data) {
        ...
        for (let i = 0; i < rewardList.length; i++) {
            ...
            tempString += `
			<tr class="table-row history-link" data-link="${encodeURIComponent(reward.name)
                .toLowerCase()
                .replace("'", "%27")}" data-title="${reward.name}" ${FLTR_ID}="${i}" id='${i}'>
				<td class='${CLS_COL_1}'>${reward.name}</td>
				<td class='${CLS_COL_2}' style='width: 73px;' title="${Parser.sourceJsonToFull(
                reward.source
            )}">${Parser.sourceJsonToAbv(reward.source)}</td>
			</tr>`;
    ...
    window.loadhash = function(id) {
        ...
        if (reward.ability !== undefined)
            texthtml += utils_combineText(
                reward.ability.text,
                "p",
                "<span class='stat-name'>Ability Score Adjustment:</span> "
            );
        if (reward.signaturespells !== undefined)
            texthtml += utils_combineText(
                reward.signaturespells.text ? reward.signaturespells.text : "None",
                "p",
