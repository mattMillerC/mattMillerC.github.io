...
(() => {
    ...
    function onJsonLoad(data) {
        ...
        PSIONIC_LIST.forEach(function(p, i) {
            ...
            tempString += `
			<tr class="table-row history-link" data-link="${encodeForHash([p[JSON_ITEM_NAME], p[JSON_ITEM_SOURCE]])}" data-title="${
                p[JSON_ITEM_NAME]
            }" ${FLTR_ID}="${i}" id='${i}'>
				<td class='${LIST_NAME} ${CLS_COL1}'>${p[JSON_ITEM_NAME]}</td>
				<td class='${LIST_SOURCE} ${CLS_COL2}' title="${Parser.sourceJsonToFull(p[JSON_ITEM_SOURCE])}">${Parser.sourceJsonToAbv(
                p[JSON_ITEM_SOURCE]
            )}</td>
				<td class='${LIST_TYPE} ${CLS_COL3}'>${parse_psionicTypeToFull(p[JSON_ITEM_TYPE])}</td>
				<td class='${LIST_ORDER} ${CLS_COL4} ${p[JSON_ITEM_ORDER] === STR_NONE ? CLS_LI_NONE : STR_EMPTY}'>${
                p[JSON_ITEM_ORDER]
            }</td>
				<td class='${LIST_MODE_LIST}' style='display:none;'>${getHiddenModeList(p)}</td>
			</tr>
		`;
    ...
    window.loadhash = function(jsonIndex) {
        const STATS_ORDER_AND_TYPE = $(".stats-wrapper .order")[0];
        const STATS_DURATION = $(".stats-wrapper .duration")[0];
        const STATS_TEXT = $(".stats-wrapper .text")[0];
        ...
        function loadDiscipline() {
            STATS_ORDER_AND_TYPE.innerHTML = `${selectedPsionic[JSON_ITEM_ORDER]} ${parse_psionicTypeToFull(
                selectedPsionic[JSON_ITEM_TYPE]
            )}`;
            ...
            function getFocusString() {
                return `<p><span class='stat-name'>Psycic Focus.</span> ${selectedPsionic[JSON_ITEM_FOCUS]}</p>`;
            }
            function getModeString(modeIndex) {
                const modeString = utils_combineText(
                    selectedPsionic[JSON_ITEM_MODES][modeIndex][JSON_ITEM_MODE_TEXT],
                    ELE_P,
                    getModeTitle(selectedPsionic[JSON_ITEM_MODES][modeIndex])
                );
                ...
                function getSubModeString() {
                    ...
                    for (let i = 0; i < subModes.length; ++i) {
                        modeStrings.push(
                            utils_combineText(subModes[i][JSON_ITEM_MODE_TEXT], ELE_P, getModeTitle(subModes[i], true))
                        );
                    }
                ...
                function getModeTitle(mode, subMode) {
                    ...
                    if (subMode) return `<span class='stat-name'>${modeTitleArray.join(STR_JOIN_MODE_TITLE)}.</span> `;
                    else return `<span class='stat-name'>${modeTitleArray.join(STR_JOIN_MODE_TITLE)}.</span> `;
                    ...
                    function getModeTitleBracketPart() {
                        ...
                        function getModeTitleConcentration() {
                            return `conc., ${
                                mode[JSON_ITEM_MODE_CONCENTRATION][JSON_ITEM_MODE_CONCENTRATION_DURATION]
                            } ${mode[JSON_ITEM_MODE_CONCENTRATION][JSON_ITEM_MODE_CONCENTRATION_UNIT]}.`;
                        }
                    }
                }
            }
