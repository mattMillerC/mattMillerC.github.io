...
function onJsonLoad(data) {
	...
	PSIONIC_LIST.forEach(function(p, i) {
		...
		tempString += `
			<tr class="table-row history-link" data-link="${encodeForHash([p[JSON_ITEM_NAME], p[JSON_ITEM_SOURCE]])}" data-title="${p[JSON_ITEM_NAME]}" ${FLTR_ID}="${i}" id='${i}'>
				<td class='${LIST_NAME} ${CLS_COL1}'>${p[JSON_ITEM_NAME]}</td>
				<td class='${LIST_SOURCE} ${CLS_COL2}' title="${Parser.sourceJsonToFull(p[JSON_ITEM_SOURCE])}">${Parser.sourceJsonToAbv(p[JSON_ITEM_SOURCE])}</td>
				<td class='${LIST_TYPE} ${CLS_COL3}'>${parse_psionicTypeToFull(p[JSON_ITEM_TYPE])}</td>
				<td class='${LIST_ORDER} ${CLS_COL4} ${p[JSON_ITEM_ORDER] === STR_NONE ? CLS_LI_NONE : STR_EMPTY}'>${p[JSON_ITEM_ORDER]}</td>
