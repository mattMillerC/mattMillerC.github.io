...
(() => {
    ...
    function onJsonLoad(data) {
        ...
        for (let i = 0; i < cultList.length; i++) {
            ...
            tempString += `
			<tr class="table-row history-link" data-link="${encodeURI(
                name
            ).toLowerCase()}" data-title="${name}" ${FLTR_ID}="${i}" id='${i}'>
    ...
    window.loadhash = function(id) {
        ...
        if (curcult.cultists !== undefined)
            texthtml += utils_combineText(
                curcult.cultists.text,
                "p",
                "<span class='stat-name'>Typical Cultist:</span> "
            );
