...
(() => {
    ...
    function onJsonLoad(data) {
        ...
        const filterBox = initFilterBox(sourceFilter, asiFilter);
        ...
        for (let i = 0; i < featlist.length; i++) {
            ...
            const CLS_COL_4 =
                "table-cell prerequisite hidden-mobile-down " + (prereqText === STR_NONE ? "list-entry-none " : "");

            tempString += `
			<tr class="table-row history-link" data-link="${encodeForHash(name) +
                HASH_LIST_SEP +
                encodeForHash(curfeat.source)}" data-title="${name}" ${FLTR_ID}="${i}" id='${i}'>
				<td class='${CLS_COL_1}'>${name}</span>
				<td class='${CLS_COL_2}' title='${Parser.sourceJsonToFull(curfeat.source)}'>${Parser.sourceJsonToAbv(
                curfeat.source
            )}</span>
                ...
                if (abilityObj.choose === undefined) {
                    ...
                    for (let i = 0; i < choose.length; ++i) {
                        if (choose[i].from.length === 6) {
                            if (choose[i].textreference === "YES") {
                                // only used in "Resilient"
                                abbArr.push(
                                    "Increase the chosen ability score by " + choose[i].amount + TO_MAX_OF_TWENTY
                                );
                            } else {
                                abbArr.push(
                                    "Increase one ability score of your choice by " +
                                        choose[i].amount +
                                        TO_MAX_OF_TWENTY
                                );
                            }
